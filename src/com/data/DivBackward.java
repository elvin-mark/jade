...
public class DivBackward extends Node {
    ...
    public void backward(Tensor loss) {
        ...
        for (int i = 0; i < loss.data.length; i++) {
            new_loss_1.data[i % new_loss_1.size] += loss.data[i] / t2.data[i % t2.size];
            new_loss_2.data[i % new_loss_2.size] += -t1.data[i % t1.size] * loss.data[i] / Math.pow(
                    t2.data[i % t2.size], 2.0f);
        }
